...
import { runGeneratePageObjectFromCli } from "../utils/generatePageObject";
...
export function buildGeneratePageObjectCommand() {
    ...
    return command
        ...
        .option("-d, --output-dir <path>", "Directory where page objects are saved when --outfile is omitted")
        .option("-s, --scope <selector>", "CSS selector to limit DOM extraction scope")
        .option("-m, --max <count>", "Maximum number of elements to include", (value) => Number.parseInt(value, 10))
        ...
        try {
            ...
            console.log(`? Generated ${result.className} with ${result.elementsGenerated} elements ? ${result.outfile}`);
        }
        catch (error) {
